...
export function Hero({ stars = null }: HeroProps) {
  return (
    ...
    // Clipping here rather than on the max-w section keeps the glow's blur from
    // being cut into a hard vertical edge beside the terminal. `clip` rather
    // than `hidden` so it does not become a scroll container (which would break
    // `position: sticky` in sections further down).
      ...
      <section id="product" className="mx-auto w-full max-w-7xl px-5 pb-14 pt-24 sm:px-7 sm:pt-28 lg:pb-16 lg:pt-28">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:items-center lg:gap-10 xl:gap-12">
          <div className="max-w-xl text-left">
            ...
            <InstallCommandCompact className="mt-6 max-w-lg" />
